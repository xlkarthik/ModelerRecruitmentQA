...
class QAJobQueue {
  ...
  async addJob(
    ...
    await supabase.from("qa_jobs").update({ status: "queued" }).eq("id", jobId).select();
    this.processQueue();
  }
...
async function processQAJob(
  ...
  try {
    ...
    if (statusError) {
      console.error(`Failed to update job ${jobId} to processing:`, statusError);
      throw new Error(`Failed to update job status: ${statusError.message}`);
    }
...
Output *only* a single valid JSON object, for example:
{
  ...
  "status": "Not Approved"
    ...
    if (updateError) {
      console.error(`❌ Failed to update job ${jobId} in database:`, updateError);
      throw new Error(`Database update failed: ${updateError.message}`);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const jobId = uuidv4();
    const { data: insertData, error: insertError } = await supabase.from("qa_jobs").insert([
      {
        id: jobId,
        status: "pending",
        start_time: new Date().toISOString(),
